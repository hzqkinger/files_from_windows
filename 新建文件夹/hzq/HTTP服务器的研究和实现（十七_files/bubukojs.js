...
$(document).ready(function() {
    var obj = $("#xuanting");
    if (obj) {
        var left = parseInt(($(document.body).width() - 1000) / 2) + 1000 - 300 + 1;
        var contentTop = parseInt(obj.offset().top);
        $(window).bind("scroll", function() {
            var tt = $(document).scrollTop() + 19;
            if (tt > contentTop && $(document).height() > contentTop + 600 + 1000) {
...
function bubuko_analysis(key) {
    $.ajax({
        type: "POST", cache: false,
        url: "/ajaxjs/info_detail_ajaxjs.aspx?act=ad_analysis&key=" + key,
        data: "",
        success: function (data) {
        }
    });
